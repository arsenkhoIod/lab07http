    ...
    const suggester_server& suggester)
{
  ...
  //при хорошем запросе отправляем:
  std::string output_to_send;
  //парсим строку (запрос) клиента
  try {
  ...
  // проверяем с каким методом был отправлен http-запрос
  if ((req.method() == http::verb::post) &&
    ...
    //отправляем заготовку при плохом запросе
...
// процесс обработки каждого клиента
void
...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    // io - in/out-server, класс в бусте
    net::io_context ioc{1};

    // The acceptor receives incoming connections
    tcp::acceptor acceptor{ioc, {address, port}};
    //бесконечный цикл, подключает клиентов
    for(;;)
    {
      // сокеты(через них идёт подключение)
      tcp::socket socket{ioc};

      // это boost-класс, непосредственно занимается подключением клиентов
      acceptor.accept(socket);

      // создаёт потоки для каждого клиента
      std::thread{std::bind(&do_session,std::move(socket), doc_root)}.detach();
    }
  }
  //если происходит ошибка, то выбрасывает сообщение
